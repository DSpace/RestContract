...
Provide detailed information about a specific community. The JSON response document is as follows:
...
```json
{
  ...
  "metadata": {
    ...
    "dc.description.abstract": [
      {
        "value": "This is a test community to hold content for the OR2017 demonstration",
        "language": null,
        "authority": null,
        "confidence": -1
      }
...
community. [See the bitstream endpoint for more info](bitstreams.md#Single Bitstream)
...
It returns the community containing this community, e.g. for trail purposes.  
...
* 403 Forbidden - if you are not logged in with sufficient permissions to retrieve the current community or parent
  community
...
community. [See the EPerson Group endpoint for more info](epersongroups.md#single-eperson-group)
...
* 401 Unauthorized - if you are not authenticated
* 403 Forbidden - if you are not logged in with sufficient permissions to retrieve the groups. Only admins and (parent)
  community admins can retrieve the group
* 404 Not found - if the current community doesn't exist
...
* 403 Forbidden - if you are not logged in with sufficient permissions. Only admins and parent community admins can
  retrieve the group
* 404 Not found - if the community doesn't exist
* 422 Unprocessable Entity - if the name was included, if permanent was set to true, or if the community already
...
* 204 No content - if the deletion succeeded (including the case of no-op if the community didn't contain an
  administrator group)
* 401 Unauthorized - if you are not authenticated
* 403 Forbidden - if you are not logged in with sufficient permissions. Only admins and (parent) community admins can
  delete the group
...
To create a top level community, perform as post with the JSON below to the communities endpoint when logged in as
admin.
...
To create a sub level community, perform as post with the JSON below to the communities endpoint when logged in as
admin.
...
Provide updated metadata information about a specific community, when the update is completed the updated object will be
